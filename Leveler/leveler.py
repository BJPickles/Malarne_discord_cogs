...
class Leveler(commands.Cog):
    ...
    async def setlevel(self, ctx, level: int, member: discord.Member = None):
        ...
        if await self.profiles._is_registered(member):
            await self.profiles._set_exp(member, 2 * ((level - 1) ** 2) + (2 * (level - 1)) + 10)
        else:
    ...
    async def addxp(self, ctx: commands.Context, xp: int, *members: discord.Member) -> None:
        ...
        success_added: str = ", ".join((member.name for member in success))
        failed_added: str = ", ".join((member.name for member in failed))
